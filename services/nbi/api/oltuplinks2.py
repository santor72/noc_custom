...
class OltUplinks2API(NBIAPI):
    ...
    def handler(self):
        try:
            ...
            resint = {}
            for item in oltint:
                if item.managed_object.bi_id in resint.keys():
                    resint[item.managed_object.bi_id]['path'].append(f"{item.name}={item.name}")
                else:
                    resint[item.managed_object.bi_id] = {'bi_id':item.managed_object.bi_id,
                                                         'name': item.managed_object.name,
                                                         'path': [f"{item.name}={item.name}"]}
            result=[]
            for item in resint:
                name=resint[item]['name']
                bi_id = resint[item]['bi_id']
                path = ', '.join(resint[item]['path'])
