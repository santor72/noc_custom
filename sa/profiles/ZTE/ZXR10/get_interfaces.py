...
class Script(BaseScript):
    ...
    rx_int2 = re.compile(r"^(?P<name>.+?)\s+is\s+(?P<status>(?:administratively )?\S+),\s+line.+$")
    ...
    def execute_cli(self, **kwargs):
        ...
        for line in self.cli("show interface").splitlines():
            ...
            if not match:
                match = self.rx_int2.search(line)
            if match:
        ...
        for iface in ifaces:
            ...
            subif["description"] = ifaces[iface].get("description")
