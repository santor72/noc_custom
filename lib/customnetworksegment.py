...
@on_save
class CustomNetworkSegment(Document):
    meta = {
    ...
    def get_by_id(cls, id):
        return CustomNetworkSegment.objects.filter(id=id).first()

    @classmethod
    @cachetools.cachedmethod(operator.attrgetter("_bi_id_cache"), lock=lambda _: id_lock)
    def get_by_bi_id(cls, id):
        return CustomNetworkSegment.objects.filter(bi_id=id).first()
    ...
    def managed_objects(self):
        ...
        q = {"bi_id__in": [v for k,v in self.ospftopo['16143']['routers'].items()]}
    ...
    def get_siblings(self, seen=None):
        ...
        seen |= ss
        for s in CustomNetworkSegment.objects.filter(sibling=self):
    ...
    def has_children(self):
        return bool(CustomNetworkSegment.objects.filter(parent=self.id).only("id").first())
    ...
    def set_redundancy(self, status):
        ...
        CustomNetworkSegment._get_collection().update_many(filter, {"$set": set_op})

    def set_lost_redundancy(self, status):
        CustomNetworkSegment._get_collection().update(
    ...
    def update_summary(cls, network_segment):
        ...
        if not hasattr(network_segment, "id"):
            network_segment = CustomNetworkSegment.get_by_id(network_segment)
        path = network_segment.get_path()
        # Update upwards
        path.reverse()
        for ns in sorted(
            CustomNetworkSegment.objects.filter(id__in=path), key=lambda x: path.index(x.id)
        ):
            r = ns.get_summary()
            CustomNetworkSegment._get_collection().update_one({"_id": ns.id}, {"$set": r}, upsert=True)
    ...
    def update_access(self):
        ...
        for s in CustomNetworkSegment.objects.filter(parent=self.id).only("adm_domains"):
    ...
    def get_nested_ids(self):
        ...
        wave = {self.id}
        max_level = 10
        coll = CustomNetworkSegment._get_collection()
    ...
    def iter_vlan_domain_segments(cls, segment):
        ...
        def iter_segments(ps):
            ...
            for s in CustomNetworkSegment.objects.filter(parent=ps.id):
                if s.vlan_border:
                    continue
    ...
    def get_total_summary(self, ids=None, parent_id=None):
        ...
        try:
            for doc in CustomNetworkSegment._get_collection().aggregate(pipeline):
